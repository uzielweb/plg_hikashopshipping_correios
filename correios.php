...
class plgHikashopshippingCorreios extends hikashopShippingPlugin {
    var $correios_methods = array(
        array('key' => 1, 'name' => 'Sedex Sem Contrato (40010)', 'countries' => 'BRAZIL'),
        array('key' => 2, 'name' => 'Sedex Com Contrato (40096)', 'countries' => 'BRAZIL'),
        array('key' => 3, 'name' => 'E-Sedex Com Contrato (81019)', 'countries' => 'BRAZIL'),
        array('key' => 4, 'name' => 'PAC Sem Contrato (41106)', 'countries' => 'BRAZIL'),
        array('key' => 5, 'name' => 'PAC Com Contrato (41068)', 'countries' => 'BRAZIL'),
        array('key' => 6, 'name' => 'Sedex 10 (40215)', 'countries' => 'BRAZIL'),
        array('key' => 7, 'name' => 'Sedex HOJE (40290)', 'countries' => 'BRAZIL'),
        array('key' => 8, 'name' => 'Sedex a Cobrar (40045)', 'countries' => 'BRAZIL'),
    );
    ...
    function onShippingDisplay(&$order, &$dbrates, &$usable_rates, &$messages) {
        ...
        foreach ($local_usable_rates as $k => $rate) {
            if (!empty($rate->shipping_params->methodsList)) {
                ...
                $messages['no_shipping_methods_configured'] = 'No shipping methods configured in the CORREIOS shipping plugin options';
                continue;
            }
            ...
            foreach ($receivedMethods as $method) {
                ...
                foreach ($this->correios_methods as $correios_method) {
                    if ($correios_method['name'] == $method['name']) {
                ...
                if ($method['deliveryDate'] != 'www.correios.com.br') {
    ...
    function _getBestMethods(&$rate, &$order, $null) {

    }

    function _getShippingMethods(&$rate, &$order, $null) {
        $data = array();
        $data['merchant_ID'] = $rate->shipping_params->merchant_ID;
        $data['merchand_PWD'] = $rate->shipping_params->merchant_PWD;
