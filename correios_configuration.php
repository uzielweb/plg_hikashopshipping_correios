	...
	<tr>
		<td class="key">
			<label for="data[shipping][shipping_params][access_code]">
				<?php echo JText::_( 'CORREIOS_ACCESS_CODE' ); ?>
			</label>
		</td>
		<td>
	...
	<tr>
		...
		<td id="shipping_services_list">
			<?php
					...
					$i=-1; foreach($this->data['correios_methods'] as $method){
					$i++;
					$varName=strtolower($method['name']);
					$varName=str_replace(' ','_', $varName);
				?>
				<input id="data_shipping_correios_<?php echo $varName;?>" name="data[shipping_methods][<?php echo $varName;?>][name]" type="checkbox" value="<?php echo $varName;?>" <?php echo (!empty($this->element->shipping_params->methods[$varName])?'checked="checked"':''); ?>/><label for="data_shipping_correios_<?php echo $varName;?>"><?php echo $method['name'].' ('.$method['countries'].')'; ?></label><br/>
			<?php	} ?>
		</td>
	</tr>
	<tr>
		<td class="key">
			<label for="data[shipping][shipping_params][format]">
				<?php echo JText::_( 'FORMAT' ); ?>
			</label>
		</td>
		<td>
			<?php
				$hikacorreiosformat = array(
					JHTML::_('select.option', 'box_pack', JText::_('BOX_PACK')),
					JHTML::_('select.option', 'roll_prism', JText::_('ROLL_PRISM')),
					JHTML::_('select.option', 'envelope', JText::_('ENVELOPE') ),
				);
				echo JHTML::_('hikaselect.genericlist', $hikacorreiosformat, "data[shipping][shipping_params][format]" , 'class="inputbox" size="1"', 'value', 'text', @$this->element->shipping_params->format);
			?>
		</td>
	</tr>
	<tr>
		<td class="key">
			<label for="data[shipping][shipping_params][own_hands]">
				<?php echo JText::_( 'OWN_HANDS' ); ?>
			</label>
		</td>
		<td>
			<?php echo JHTML::_('hikaselect.booleanlist', "data[shipping][shipping_params][own_hands]" , '',@$this->element->shipping_params->own_hands); ?>
		</td>
	</tr>
	<tr>
		<td class="key">
			<label for="data[shipping][shipping_params][declared_value]">
				<?php echo JText::_( 'DECLARED_VALUE' ); ?>
			</label>
		</td>
		<td>
			<?php echo JHTML::_('hikaselect.booleanlist', "data[shipping][shipping_params][declared_value]" , '',@$this->element->shipping_params->declared_value); ?>
		</td>
	</tr>
	<tr>
		<td class="key">
			<label for="data[shipping][shipping_params][receipt]">
				<?php echo JText::_( 'RECEIPT' ); ?>
			</label>
		</td>
		<td>
			<?php echo JHTML::_('hikaselect.booleanlist', "data[shipping][shipping_params][receipt]" , '',@$this->element->shipping_params->receipt); ?>
		</td>
	</tr>
</table>
</fieldset>
